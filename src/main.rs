...
fn main() {
    ...
    let (generator_ctx, txn_generator, finished_txn_chan) = generator::new(&blockchain, &mempool);
    let generator_worker_ctx = generator::generator::TransactionGenerator::new(&server, finished_txn_chan, &mempool);
    generator_ctx.start();
